...
```{r}
mtcars_tibble_filtered <- as.tibble(mtcars, rownames = "model") %>% 
  filter(cyl == 6) %>% 
  select(model, mpg, hp)  
mtcars_tibble_filtered 
...
```{r}
knitr::kable(mtcars_tibble_filtered)
...
colnames(iris_lower) <- c("sepal_length", "sepal_width", "petal_length", "petal_width", "species") %>% 
  
iris_lower <- iris_lower %>% 
  select(species, everything())

## OR:

## new name = old name format for renaming the variables
...
```{r}
iris_lower %>% 
  ggplot(aes(species, sepal_width, alpha = 0.9)) +
  geom_boxplot() +
  geom_jitter(aes(color = species)) +
  xlab("Species") +
  ylab("Sepal Width") +
  ggtitle("Sepal Width of Flower Species") +
  theme_bw() +
...
iris_inch <- (iris_lower / 2.54) 
iris_inch

# (this one does not have species name)
...
##color and shape functions apply random shapes and colors to each of the variables selected
...
## this will create a new variable column and will display "true" if life exp is less than 60

```



## practicing with group_by and summarize()
...
gapminder %>% 
 ...
 ## using n_distinct will pick out the distinct variables (country, in this case) under each continent, renamed to a new column called "n_countries." m_lifeExp is also a new variable created under the summarize function
```


```{r}
## increase in gdp per capita for each country
...
```{r}
iris %>%
  mutate(Petal.Length = round(Petal.Length))
```
...
edu_level <-
  psych::bfi %>% 
    ...
    _mn = rowMeans(across(C1:C5)), 
    A_mn = rowMeans(across(E1:E5)), 
    A_mn = rowMeans(across(N1:N5)), 
    A_mn = rowMeans(across(O1:O5)))
