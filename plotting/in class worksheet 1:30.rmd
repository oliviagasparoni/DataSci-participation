...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  geom_point(alpha = 0.1)
```
# Changing the scale on a plot, changes x-axis label and provides a dollar format as well to plot the data as dollar values
...
The code puts the x and y axis in the wrong spot, it should go in the parentheses next to ggplot with an aes function as well. 
