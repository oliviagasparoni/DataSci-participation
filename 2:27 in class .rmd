...
# you want to keep the ones that match and get rid of the ones that don't have matches. the list will filter for song and artist name
```
...
# we only want to keep the songs where we have year information
```
...
# we want the ones that dont have a matxh
```
...
#this will duplicate the artist name because it appears in both. to prevent this:
...
#this will filter to only ones in seattle wa:
time %>% 
...
#this works because we have the same columns in all three of the data sets

#or 

union_all(fell, ttow, retk)
...
flights %>% 
  ...
  left_join(airports, by = c("dst", "faa") %>% 
